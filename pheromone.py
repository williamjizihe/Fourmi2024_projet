...
class Pheromon:
    ...
    def mark(self, the_position, pheromon_old, has_WESN_exits):
        assert(the_position[0] >= 0)
        assert(the_position[1] >= 0)
        cells = np.array([pheromon_old[the_position[0]+1, the_position[1]] if has_WESN_exits[d.DIR_WEST] else 0.,
                          pheromon_old[the_position[0]+1, the_position[1]+2] if has_WESN_exits[d.DIR_EAST] else 0.,
                          pheromon_old[the_position[0]+2, the_position[1]+1] if has_WESN_exits[d.DIR_SOUTH] else 0.,
                          pheromon_old[the_position[0], the_position[1]+1] if has_WESN_exits[d.DIR_NORTH] else 0.], dtype=np.double)
